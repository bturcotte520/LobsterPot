...
if (import.meta.url === `file://${process.argv[1]}`) {
    ...
    if (!existing) {
      ...
      runtime.pushRelay.register(bridgeId).then((reg) => {
        if (reg) {
          ...
          console.warn(
            "Push relay registration failed — push notifications disabled until relay is reachable."
          );
        }
      });
    } else {
  ...
  const server = serve(
    { fetch: runtime.app.fetch, port: runtime.config.port },
    (info) => {
      console.log(`LobsterPot bridge listening on http://127.0.0.1:${info.port}`);
      if (!runtime.pushRelay) {
        console.log(
          "Push relay not configured (set LOBSTERPOT_PUSH_RELAY_URL, _TOKEN, and LOBSTERPOT_APNS_BUNDLE_ID)."
        );
      }
    }
  );
